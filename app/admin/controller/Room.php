...
use think\facade\Db;
 ...
 class Room extends Base{
    ...
    public function room_list()
    {
        $map = [];
        $keyword = input('param.keyword');
        $map[] = ['rd.room_num','like','%'.$keyword.'%'];
        $Nowpage = input('get.page')?input('get.page'):1;
        $limits = input('get.limit',10);
        $count = Db::name('room_detalis')->alias('rd')->where($map)->count();
        $allPage = intval(ceil($count/$limits));
        $lists = Db::name('room')->alias('rd')->field('rd.id,rd.desc,rd.area,rd.people_num,rd.sort,rd.create_time,rd.update_time,floor.floor_num,room.type,room_status.status,room_equipment.equipment')
                                 ...
                                 ->join('room_equipment','room_equipment.id=rd.rq_id','left')
